...
export async function GET() {
  try {
    ...
    const response = await serverPost<{ isSubmitted: boolean; isCorrect: boolean | null }>("/progress/daily/status", {
      language,
    });
    return NextResponse.json(response);
  } catch (_error) {
    return NextResponse.json({ error: { message: "Failed to fetch daily challenge status" } }, { status: 500 });
  }
}
